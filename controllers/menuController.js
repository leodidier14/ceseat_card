const path = require('path')
require('dotenv').config({ path: path.resolve(__dirname, '../.env') })
var db = require('../database')
var Sequelize = require('sequelize')

const menuModel = require('../models/menus')
const menuArticlesModel = require('../models/menusArticles')

class menuController {

    async addMenu(req,res){
        menuModel.create({
                                "restaurantId":req.body.restaurantId,
                                "name":req.body.name,
                                "description":req.body.description,
                                "price":parseFloat(req.body.price.toString().replace(',','.')).toFixed(2),
                                "pictureLink":req.body.image
                            })
        .then( async result => { await db.query('SELECT @@IDENTITY', {type: Sequelize.QueryTypes.SELECT})
                            .then(id => {
                                
                                req.body.articles.forEach( article => {
                                    menuArticlesModel.create({"menuId":id[0][''],"articleId":article.id})
                                })
                            })
                            res.status(200).send(result)
        })
        .catch(error => {res.status(400).send(error)})
    }

    async editMenu(req,res){
        const men = await menuModel.update({
            "name":req.body.name,
            "description":req.body.description,
            "price":parseFloat(req.body.price.toString().replace(',','.')).toFixed(2),
            "pictureLink":req.body.pictureLink
        },{
            where:{
                "id":req.body.id
            }
        }
        )
        .then(result => {
            menuArticlesModel.destroy({where: {'menuId': req.body.id}}).then( async() => {
            await req.body.articles.forEach( article =>{
                    menuArticlesModel.create({"menuId":req.body.id,"articleId":article.id}).catch(error => { 
                        res.status(400).send(error)})
                }
            );
            res.send(200).send('menu add')
        })
            }
        ).catch(error => {res.status(400).send(error)})
        
        
    }

    async deleteMenu(req,res){
        menuModel.update({
            "restaurantId":null,
        },{
            where:{
                "id":req.params.menuId
            }
        }
        )
        .then(result => res.status(200).send(result))
        .catch(error => {res.status(400).send(error)})
    }
}

module.exports = new menuController()